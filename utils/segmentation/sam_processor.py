...
class SAMClient:
    ...
    def __init__(self):
        ...
        logging.debug(f"SAMClient initialized with model from {self.model_path}")
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            logging.error(f"Failed to load SAM model from {self.model_path}: {e}")
    ...
    def segment_with_boxes(
        ...
        Returns:
            ...
            If return_structured=True: Updated base_data with contours integrated
        """
        ...
        if width <= 0 or height <= 0:
            logging.error(f"Invalid image dimensions: {width}x{height}. Cannot perform segmentation.")
            if return_structured and base_data:
                return update_data_with_contours(base_data, {})
            return {}
        ...
        for item in bounding_boxes:
            if not isinstance(item, dict):
                logging.pii(f"Skipping non-dictionary item in bounding box list: {item}")
                continue
            ...
            if not label or not isinstance(label, str):
                logging.pii(f"Skipping item with missing or invalid label: {item}")
                continue
            
            if not bbox or not isinstance(bbox, (list, tuple)) or len(bbox) != 4:
                logging.pii(f"Skipping item with missing or invalid 'bbox_2d': {item}")
                continue
        ...
        try:
            ...
            if use_prompts:
                ...
                # Note: The exact parameter name may vary by ultralytics version
                results = self.model(image, bboxes=bboxes, labels=labels)
                logging.debug(f"Running SAM with {len(bboxes)} boxes and text prompts")
            else:
            ...
            if aggregate_by_label:
                ...
                for i, label in enumerate(labels):
                    try:
                        if i < len(results[0]):
                            ...
                            logging.pii(f"Extracted {len(normalized_contours)} contours for label '{label}'")
                        else:
                            logging.warning(f"No result for label '{label}' at index {i}")
                    except Exception as e:
                        logging.pii(f"Error processing result for label '{label}': {e}", exc_info=True)
                        continue
            else:
                ...
                for i, label in enumerate(labels):
                    try:
                        ...
                        logging.pii(f"Error processing result {i} for label '{label}': {e}", exc_info=True)
                        continue
        ...
        if return_structured and base_data:
            return update_data_with_contours(base_data, aggregated_contour_data)
        else:
    ...
    def segment_stages(
        ...
        This method maintains backward compatibility with the original function signature.
    ...
    def warmup(self) -> bool:
        ...
        try:
            ...
            logging.pii(f"SAM warmup failed: {str(e)}")
            return False
...
def segment_stages(
    ...
    """
    Backward-compatible function matching the original segment_stages signature.
...
def segment_image_with_boxes(
    ...
    """
    Convenience function to segment an image without creating a client instance.
