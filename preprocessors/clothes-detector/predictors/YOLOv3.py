...
from yolo.utils.models import Darknet
from yolo.utils.utils import load_classes, non_max_suppression, rescale_boxes
...
class YOLOv3Predictor(object):
    ...
    def load_model(self):
        ...
        model = Darknet(
            ...
            img_size=self.params['img_size']).to(
            self.params['device'])
    ...
    def cv_img_to_tensor(self, img, dim=(416, 416)):
        ...
        x = F.interpolate(x, size=(ih, iw), mode='bilinear',
                          align_corners=False)  # x = (1, 3, 416, 416)
