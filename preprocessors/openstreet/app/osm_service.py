...
def query_osmdata(radius: float, lat: float, lon: float):
    api = overpy.Overpass()
    result = api.query(
        f'way(around:{radius},{lat},{lon})[highway~"^(primary|tertiary|residential|service|footway)$"];(._;>;);out body;'
    )
...
def get_points_of_interest(radius: float, lat: float, lon: float):
    ...
    api = overpy.Overpass()
    result1 = api.query(
        f'node(around:{radius},{lat},{lon})["amenity" = ""];(._;>;);out body;'
    )
    return result1
...
def merge_street_by_name(transformed_osmdata: List[dict]):
    ...
    for obj in response:
        ...
        if str_name not in output:
            ...
            record = {
                "street_id": random.randint(10_000_000, 99_999_999),
...
def extract_nodes_list(result):
    ...
    for i in range(len(result)):
        for j in range(i + 1, len(result)):
            ...
            if len(intersection):
                str_record = {
                    "street_name": result[i]["street_name"],
                ...
                str_record = {
                    "street_name": result[j]["street_name"],
                    "intersection_sets": intersection,
                }
                str_link.append(str_record)
...
def my_final_data_structure(
    merged_street, modified_intersection_set, merged_intersection
):
    str_records = []
    for obj in range(len(merged_street)):
        ...
        for i in range(len(nodes)):
            if nodes[i] not in modified_intersection_set:
                ...
                alist.append(node)
                str_info = {
                    "street_name": merged_street[obj]["street_name"],
                    "nodes": alist,
                }
