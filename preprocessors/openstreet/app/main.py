...
def get_location(distance_in_metres: float, lat: float, lon: float):
    ...
    merged_intersection = merge_street_intersection_by_name(
    ...
    street_records = my_final_data_structure(
        copy_of_merged_street,
        copy_of_new_intersection_list,
        merged_intersection)
    ...
    ) = keep_in_list_all_retrieved_point_of_interest(retrieved_point_of_interest)  # noqa: E501

    response = final_osm_data_format(
        unique_listed_point_of_interest, merged_street)
