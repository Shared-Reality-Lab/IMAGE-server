...
def get_data(line, plot_area, min_value, max_value):
    data = []
    for group in line:
        x = group[0]
        ...
        data.append([frac_x, min_value + frac_y*(max_value - min_value)])
...
def GroupLine(image, keys, hybrids, plot_area, pair_info,  min_value,  max_value):
    ...
    for line in union_result:
        ...
        if len(line) > 1:
            ...
            pixel_points.append(data_line.copy())
            data_line = get_data(data_line, plot_area, min_value, max_value)
