...
def kp_decode(nnet, inputs, K, cuda_id, ae_threshold=0.5, kernel=3):
    with torch.no_grad():
            detections, time_backbone, time_psn = nnet.test(inputs, cuda_id, ae_threshold=ae_threshold, K=K, kernel=kernel)
            #print(detections)
...
def kp_detection(image, db, quiry, nnet, cuda_id, debug=False, decode_func=kp_decode):
    ...
    if True:
        ...
        predictions, flag = decode_func(nnet, [images, tags, weights, tag_masks], K, cuda_id, ae_threshold=ae_threshold,
                                        kernel=nms_kernel)
        #print(predictions)
...
def testing(image, db, quiry, nnet, cuda_id, debug=False, decode_func=kp_decode):
