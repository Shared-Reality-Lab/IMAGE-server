...
from datasetsChanged import LoadStreams, LoadImages
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        if save_img or save_crop or view_img:
                            ...
                            centre = [abs((xleft+xright)/2),abs((yleft+yright)/2)]
                            area = abs(xleft-xright)*abs(yleft-yright)
                            dictionary = {
                                "ID": i, "type": str(label[:-4]), "dimensions": [xleft,yleft,xright,yright], "confidence": np.float64(label[-4:]),"centroid": centre,"area":area
                            }
