...
import segmentAudioHapticsJSON from "./segmentaudiohaptics.schema.json";
...
app.post("/handler", async (req, res) => {
    ...
    // Going ahead with SimpleAudio and/or SegmentAudioHaptics
    ...
    // Sort contour points
    const hapticInfo: { centroids: number[]; coordinates: number[]; }[] = [];
    for (let i = 0; i < segments.length; i++) {
        ...
        // push coordinate info for haptics
        const r = {
            "centroids": center,
