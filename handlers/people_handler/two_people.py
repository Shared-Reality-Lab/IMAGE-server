...
def calculate_dominant_emotion_for_two(happy,sad,neutral):
    print("two_people,12 {}".format(happy,sad,neutral))
    if(happy==2):
...
# testing: add inanimate rendering to 2 people
def rendering_for_two_people(object_emotion_inanimate,rendering,preprocessors): 
    common = common_check(object_emotion_inanimate[0]["inanimate"],object_emotion_inanimate[1]["inanimate"])
    emo_count = 0
    clothes_count = 0
    cloth0 = ""
    cloth1 = ""
    ...
    if(celeb0!='None'):
        ...
        if(object_emotion_inanimate[0]["emotion"]["emotion"]!='None'):
            rendering += " having a " + object_emotion_inanimate[0]["emotion"]["emotion"] + " expression, "
            emo_count += 1
    ...
    if(celeb1!='None'):
        ...
        if((len(cloth1)>0 and (cloth1 != " , " or cloth1 != " and ")) or object_emotion_inanimate[1]["emotion"]["emotion"]!='None'):
            if(object_emotion_inanimate[0]["emotion"]["emotion"]!='None'):
                rendering += " having a " + object_emotion_inanimate[1]["emotion"]["emotion"] + " expression, "
                emo_count += 1
            if(len(cloth0)>0 and cloth0 != " , "):
                rendering = rendering + " wearing " + cloth1 
                clothes_count += 1
    elif(celeb0 == 'None' and celeb1 == 'None'):
        if(object_emotion_inanimate[1]["emotion"]["emotion"]!='None' or  object_emotion_inanimate[0]["emotion"]["emotion"]!='None'):
            if("happy" in emotion0 or "happy"in emotion1):
                ...
                sad = sad + 1
            emo_combined = calculate_dominant_emotion_for_two(happy,neutral,sad)
            if(len(emo_combined)>0):
                rendering = rendering + " having a " + emo_combined
                emo_count += 1
        if((len(cloth1)>0 and (cloth1 != " , " or cloth1 != " and ")) or (len(cloth0)>0 and (cloth0 != " , " or cloth0 != " and "))):
            rendering += " wearing "
            clothes_count += 1
            print(cloth0)
            print(cloth1)
