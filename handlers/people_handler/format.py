...
def get_ideal_format(objects, emotion,  preprocessors):
    ...
    left2right_object = []
    try:
        position = preprocessors['ca.mcgill.a11y.image.preprocessor.position']['data']
    except:
        position = [{"id":None,"position":None}]
    objects,left2right = remove_low_confidence(preprocessors["ca.mcgill.a11y.image.preprocessor.objectDetection"]['objects'],preprocessors["ca.mcgill.a11y.image.preprocessor.sorting"]["leftToRight"])
    for i in range(len(left2right)):
        for j in range(len(objects)):
            if(left2right[i] == objects[j]["ID"]):
                if("person" in objects[j]["type"] and  objects[j]["area"]*100>4.0 and objects[j]["confidence"]*100>30.0):
...
def inanimated_interaction(object_emotion,objects):
    ...
    iou = []
    for i in range(len(objects)):
        if("person" in objects[i]["type"] or (objects[i]["confidence"]*100)<=50.0 ):
            continue
    ...
    for i in range(len(object_emotion)):
        ...
        for j in range(len(inanimate_obj)):
            ...
            if (ar is not None):
                object_emotion_inanimate[i]["inanimate"].append(inanimate_obj[j])
    return object_emotion_inanimate

def get_original_format(preprocessors):
    emotion = preprocessors["ca.mcgill.a11y.image.preprocessor.emotion"]["person_emotion"]
    cloth = preprocessors["ca.mcgill.a11y.image.preprocessor.clothesDetector"]["clothes"]
    celeb = preprocessors["ca.mcgill.a11y.image.preprocessor.celebrityDetector"]["celebrities"]
    data = []
    for i in range(len(emotion)):
        json_data = {}
    ...
    preprocessors["ca.mcgill.a11y.image.preprocessor.originalFormat"] = {"data":data}
...
def format_json(objects,change,preprocessors):
    ...
    if("ca.mcgill.a11y.image.preprocessor.emotion" in preprocessors):
        emotion = preprocessors["ca.mcgill.a11y.image.preprocessor.originalFormat"]["data"]
        emotion_flag,cloth_flag, no_none_emotion = rendering_emotion(emotion)
        if(emotion_flag==True):
            object_emotion = get_ideal_format(objects,no_none_emotion,preprocessors)
            object_emotion_inanimate = inanimated_interaction(object_emotion,objects)
        else:
            object_emotion_inanimate = []
    return multiple_flag,emotion_flag,object_emotion_inanimate,objects,just_person_count,cloth_flag