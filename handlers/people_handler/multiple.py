...
def calculate_dominant_emotion(happy,sad,neutral):
    ...
    if(happy>=1 and sad >=1 and neutral >=1):
        return "There is a mix of different emotions for these people. Some seem to be happy while others seem to have neutral or sad expression. "
    ...
    for i in range(len(object_emotion_inanimate)):
        ...
        for j in range(len(inanimate)):
            if(inanimate[j]["type"] in inanimate_dict):
                inanimate_dict[inanimate[j]["type"]] = inanimate_dict[inanimate[j]["type"]] + 1
            else:
                inanimate_dict[inanimate[j]["type"]] = 1
    inanimate_dict = dict(sorted(inanimate_dict.items(), key=lambda item: item[1], reverse = True))
    if(len(inanimate_dict)>0):
        rendering += "Among these people, "
    i = 0
    multiple_flag = 0
    single_flag = 0
    # print(inanimate_dict)
    printed=0
...
def rendering_for_multiple_people(object_emotion_inanimate,rendering,preprocessors):
    celeb_position = []
    unique_cloth = []
    emotion_count = 0
    clothes_count =0
    ...
    if(len(celeb_position)==0):
        ...
        for i in range(len(object_emotion_inanimate)):
            if(object_emotion_inanimate[i]["clothes"] !='None'):
                for j in range(len(object_emotion_inanimate[i]["clothes"])):
                    if(object_emotion_inanimate[i]["clothes"][j]["article"] !='None'):
                        logging.critical(object_emotion_inanimate[i]["clothes"][j]["article"])
                        logging.critical(unique_cloth)
                        if(any(object_emotion_inanimate[i]["clothes"][j]["article"] in s for s in unique_cloth)):
                            continue
                        else:
                            unique_cloth.append(object_emotion_inanimate[i]["clothes"][j]["article"])
                            cloth = cloth + object_emotion_inanimate[i]["clothes"][j]["color"] + " " 
                            cloth += object_emotion_inanimate[i]["clothes"][j]["article"] 
                            cloth += " , "
                            clothes_count+=1
            if(clothes_count>=3):
                cloth += " etc. "
        ...
        while(i_list<len(object_emotion_inanimate)):
            if(i_list in celeb_position):
                if(object_emotion_inanimate[i_list]["emotion"]["emotion"] is not None):
                    rendering = rendering + " a " + object_emotion_inanimate[i_list]["emotion"]["emotion"] + " faced "
                    emotion_count += 1
                rendering = rendering + object_emotion_inanimate[i_list]["celebrity"]["name"] + " , " 
                emotion_count += 1
            i_list = i_list+1
            render_add +=1
