...
def contains_conj(depSet):
    return "and" in depSet or "or" in depSet or "nor" in depSet or \
           "but" in depSet or "yet" in depSet or \
        "so" in depSet or "for" in depSet
...
# get grammatical objects for a given set of dependencies (including
# passive sentences)
...
# return the verb to the right of this verb
# in a CCONJ relationship if applicable
# returns a tuple, first part True|False and second part the modified verb
# if True
...
def _get_all_objs(v, is_pas):
    ...
    objs = [
        tok for tok in rights if tok.dep_ in OBJECTS or (
            is_pas and tok.dep_ == 'pobj')]
    objs.extend(_get_objs_from_prepositions(rights, is_pas))

    # potentialNewVerb, potentialNewObjs = _get_objs_from_attrs(rights)
    # if potentialNewVerb is not None and
    # potentialNewObjs is not None and len(potentialNewObjs) > 0:
    ...
    if potential_new_verb is not None and \
        potential_new_objs is not None and len(
...
# return true if the sentence is passive - at he moment a sentence is
# assumed passive if it has an auxpass verb
...
def printDeps(toks):
    for tok in toks:
        print(
            tok.orth_, tok.dep_, tok.pos_, tok.head.orth_, [
...
# find verbs and their subjects / objects to create SVOs, detect
                            ...
                            svos.append(
                                (to_str(
                                    expand(
                                        ...
                                        visited)),
                                    "!" +
                                    v.lemma_ if verbNegated or
                                 objNegated else v.lemma_,
                                    to_str(
                                    expand(
                                        sub,
                                        tokens,
                            ...
                            svos.append(
                                (to_str(
                                    expand(
                                        ...
                                        visited)),
                                    "!" +
                                    v2.lemma_ if verbNegated or
                                 objNegated else v2.lemma_,
                                    to_str(
                                    expand(
                                        sub,
                                        tokens,
                            ...
                            svos.append(
                                (to_str(
                                    expand(
                                        ...
                                        visited)),
                                    "!" +
                                    v.lower_ if verbNegated or
                                 objNegated else v.lower_,
                                    to_str(
                                    expand(
                                        obj,
                                        tokens,
                            ...
                            svos.append(
                                (to_str(
                                    expand(
                                        ...
                                        visited)),
                                    "!" +
                                    v2.lower_ if verbNegated or
                                 objNegated else v2.lower_,
                                    to_str(
                                    expand(
                                        obj,
                                        tokens,
